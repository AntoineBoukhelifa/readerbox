...
<main class="mx-auto max-w-2xl px-6 py-16">
	...
	<h1 class="mt-6 text-2xl font-semibold tracking-tight">Chercher</h1>
	...
	<form method="GET" class="mt-6 flex flex-wrap gap-2">
		<input
			name="q"
			value={data.requete}
			placeholder="Immortal X-Men"
			class="min-w-0 flex-1 rounded-md border border-neutral-300 px-3 py-1.5 text-sm"
		/>
		<button class="rounded-md border border-neutral-300 bg-white px-3 py-1.5 text-sm font-medium">
			Chercher
		</button>
	</form>
	...
	{:else if data.resultats.length === 0}
		<p class="mt-8 text-sm text-neutral-500">Rien sous ce nom.</p>
	{:else}
						...
						<p class="mt-1 text-xs text-neutral-400">
							...
							{#if resultat.serieSource && resultat.serieIdExterne}
								...
								<a
									...
									class="underline underline-offset-4">Toute la série</a
								>
							{/if}
						</p>
